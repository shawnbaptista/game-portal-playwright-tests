...
export class Coordinates {
    ...
    constructor(public readonly y: number, public readonly x: number) {
        if (( y > 7 || y < 0 ) || (x > 7 || x < 0) ){
            throw new RangeError(`Must provide coordinates representing a column and row that is within the 8X8 board: ${y},${x}`)
        }
    }
}
...
class CheckersBoard {
    ...
    private getSquareLocatorByCoordinates(coordinates: Coordinates) {
        const square = this.page.locator(`img[name="space${coordinates.y}${coordinates.x}"]`) 
        if (square === null) {
            throw new Error(`No square was found based on the provided coordinates: ${coordinates}`)
        }
        return square
    }

    private async parseSquareByImgLocator(imgLocator: Locator): Promise<SquareState> {
        ...
        let coordinates: Coordinates = new Coordinates(Number(rawCoordinates[0]), Number(rawCoordinates[1]))
        ...
        if (!imgSrc.includes('.gif')) {
            throw new Error(`Images in the board are expected to be of file format .gif: ${imgSrc}`)
        }
        ...
        if (imgSrc.includes('black')) {
            return { valid: valid, piece: null, selected: selected, coordinates: coordinates}
        }
        ...
        if (imgSrc.includes('you')) {
            ...
            throw new Error(`Unable to determine player based on img src of ${imgSrc}`)
        }
        ...
        return { valid: valid, piece: {type: pieceType, player: player}, selected: selected, coordinates: coordinates}
    }
    ...
    getBoardSummary(boardState: BoardState) {
        ...
        const computerPieces = playerPieces.filter( (square) => square.piece?.player === 'computer' )
        const computerPieceCount = computerPieces.length
        const userPieces = playerPieces.filter( (square) => square.piece?.player === 'user')
        const userPieceCount = userPieces.length

        // Empty
        const emptyValidSquares = boardState.filter( (square) => square.valid === true && square.piece === null)
    ...
    async makeLegalMove(boardState: BoardState) {
        ...
        let endingPosition: Coordinates;
        for (let s = 0; s < emptyValidSquares.length; s++) {
            ...
            for (let p = 0; p < userPieces.length; p++) {
                ...
                if (userPieceCoordinates.y == validSquareCoordinates.y - 1 && userPieceCoordinates.x == validSquareCoordinates.x - 1) {
        ...
        expect(startingPosition).toBeDefined()
        expect(endingPosition).toBeDefined()
        await this.clickOnSquareByCoordinates(startingPosition)
        await this.clickOnSquareByCoordinates(endingPosition)
        // TODO -- BAPTISTA: Not properly waiting for computer's turn to finish
        let endingSquare = await this.getSquareStateByCoordinates(endingPosition)
    ...
    /**
     * Board is inverted with positions in zero-based index -- bottom-right is 0,0; top-left is 7,7
     * @param column 
     * @param row 
     */
    async moveToPosition(coordinates: Coordinates) {

    }

    // Valid squares have an 'onClick' attribute: img[onClick] -- there are 32 of them
